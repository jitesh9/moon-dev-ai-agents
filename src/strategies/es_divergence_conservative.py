"""
ES Divergence Strategy - CONSERVATIVE (11% Max DD Constraint)
=============================================================
...
class DivergenceConservativeStrategy(Strategy):
    """
    Conservative divergence strategy with 11% max DD constraint.
...
def find_optimal_scale(df: pd.DataFrame, capital: float = 500000, target_dd: float = 11.0):
    ...
    for scale in np.arange(0.50, 0.65, 0.01):
        class TestStrategy(DivergenceConservativeStrategy):
...
def run_backtest(df: pd.DataFrame, capital: float = 500000):
    ...
    class OptimalStrategy(DivergenceConservativeStrategy):
    ...
    if max_dd <= 11.0:
        ...
        print("Consider reducing position_scale to 55%")
