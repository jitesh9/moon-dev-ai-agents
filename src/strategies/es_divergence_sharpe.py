"""
ES Divergence-Based High Sharpe Strategy
=========================================
Adds RSI/MACD divergence detection - the key pattern from winning 2.0+ Sharpe strategies.

A bullish divergence occurs when:
- Price makes a LOWER low
- But RSI or MACD makes a HIGHER low

This signals exhaustion of selling pressure and potential reversal.
...
def prepare_signals(df_1m: pd.DataFrame) -> pd.DataFrame:
    """
    Aggregate to HOURLY and prepare divergence-based signals.
    """
    # Resample to hourly
...
class DivergenceSharpeStrategy(Strategy):
    """
    Divergence-Based High Sharpe Strategy.
    ...
    target_mult = 2.5     # ATR multiplier for target (lower for higher win rate)
...
def run_backtest(df: pd.DataFrame, optimize: bool = False):
    print("\n" + "=" * 70)
    print("ES DIVERGENCE-BASED SHARPE STRATEGY")
