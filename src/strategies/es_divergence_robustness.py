"""
ES Divergence Strategy - Robustness Testing Suite
==================================================
...
# Import strategy components from original file
from es_divergence_sharpe import (
    load_databento_data, prepare_signals, calculate_supertrend,
    detect_divergence, detect_swing_lows, DivergenceSharpeStrategy,
    calculate_cagr
)


def run_single_backtest(df: pd.DataFrame, params: dict = None) -> dict:
    """Run a single backtest with given parameters."""
    class TestStrategy(DivergenceSharpeStrategy):
        pass
...
def walk_forward_analysis(df: pd.DataFrame, n_splits: int = 5) -> Dict:
    ...
    for i in range(n_splits):
        ...
        # Run backtest on test period with default params
        try:
...
def parameter_sensitivity(df: pd.DataFrame) -> Dict:
    ...
    # Baseline parameters (from optimized results)
    baseline_params = {
        'stop_mult': 1.5,
        'target_mult': 2.5,
        'trail_start_pct': 1.5,
    ...
    baseline = run_single_backtest(df, baseline_params)
    print(f"\nBaseline Sharpe: {baseline['sharpe']:.3f}")

    results = {'baseline': baseline, 'variations': {}}

    # Test each parameter with +/- 20% variation
    param_variations = {
    ...
    for param, values in param_variations.items():
        ...
        for value in values:
            ...
            try:
                stats = run_single_backtest(df, test_params)
    ...
    # Calculate sensitivity score (lower is better)
    all_sharpes = []
...
def monte_carlo_analysis(df: pd.DataFrame, n_simulations: int = 100) -> Dict:
    ...
    Tests if returns depend on specific trade sequence.
    """
    print("\n" + "=" * 70)
    print("MONTE CARLO SIMULATION")
    print("=" * 70)

    # First get the actual trade results
    bt = Backtest(
        df,
        DivergenceSharpeStrategy,
        cash=50000,
        commission=0.00005,
        exclusive_orders=True,
        trade_on_close=True
    ...
    for sim in range(n_simulations):
        ...
        # Approximate Sharpe (assuming daily-ish returns)
    ...
    results = {
        ...
        'dd_5pct': np.percentile(max_drawdowns, 5),  # Worst case
        'dd_median': np.percentile(max_drawdowns, 50),
        'dd_95pct': np.percentile(max_drawdowns, 95),  # Best case
    ...
    # Determine if actual is within reasonable range
    return_lucky = results['actual_return'] > results['return_95pct']
    sharpe_lucky = results['actual_sharpe'] > results['sharpe_95pct']

    print("\n" + "-" * 40)
    if return_lucky or sharpe_lucky:
        print("WARNING: Actual results may be sequence-dependent (lucky ordering)")
    else:
...
def regime_analysis(df: pd.DataFrame) -> Dict:
    ...
    # Calculate rolling returns to identify regimes
    ...
    daily_df['volatility_20d'] = daily_df['Close'].pct_change().rolling(20).std()
    ...
    for regime in ['bull', 'bear', 'sideways']:
        ...
        # Drop regime column for backtest
...
def yearly_analysis(df: pd.DataFrame) -> Dict:
    """
    Test performance year by year to check consistency.
    """
    print("\n" + "=" * 70)
    ...
    if results:
        ...
        consistency = sum(1 for r in results.values() if r['sharpe'] > 0) / len(results)
        print(f"  Consistency: {consistency*100:.1f}% positive Sharpe years")
...
def generate_robustness_report(df: pd.DataFrame) -> Dict:
    ...
    print("\n" + "=" * 70)
    print("ES DIVERGENCE STRATEGY - ROBUSTNESS TESTING")
    print("=" * 70)
