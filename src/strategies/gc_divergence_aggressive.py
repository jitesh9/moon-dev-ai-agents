"""
GC (Gold Futures) Divergence Strategy - AGGRESSIVE (No DD Constraint)
=====================================================================
...
- Higher volatility than equity indices during risk-off periods
...
class GCDivergenceAggressiveStrategy(Strategy):
    """
    Aggressive divergence strategy for Gold - maximum position sizing, no DD limits.
    """
    # Risk parameters
    stop_mult = 1.5
    target_mult = 2.5
...
def run_backtest(df: pd.DataFrame, capital: float = 500000):
    """Run aggressive backtest on GC."""
    print("\n" + "=" * 70)
    print("GC (GOLD) DIVERGENCE STRATEGY - AGGRESSIVE (NO DD LIMIT)")
    print("=" * 70)
    ...
    bt = Backtest(
        df,
        GCDivergenceAggressiveStrategy,
        cash=capital,
        commission=0.00005,
